...
test.describe('Logout — auth residue prevention (#8)', () => {
  ...
  test('① back-navigation after logout does not restore an authenticated admin page (no-store is the real mechanism)', async ({
    ...
    // PR-3 note: goBack() can legitimately return null in headless Chromium / no-bfcache
    // environments (Playwright catches the navigation error via .catch(() => null)).
    // The `if (backResp)` guard is intentional — the primary security assertion is the
    // fresh page.goto('/admin/dashboard') below, which verifies that the cookie is cleared
    // regardless of whether a goBack response is available.
    // Changing this to `expect(backResp).not.toBeNull()` would cause false CI failures
    // in headless environments where Playwright cannot produce a back-navigation response.
