...
describe('RejectUserUseCase (signature)', () => {
  ...
  it('is constructible with UserRepository + AuditLogRepository', () => {
    const uc = new RejectUserUseCase(userRepo, auditLogRepo)
    expect(uc).toBeInstanceOf(RejectUserUseCase)
  })
  ...
  it('returns Result.err(ExternalServiceError) "not implemented: Step 3"', async () => {
    const uc = new RejectUserUseCase(userRepo, auditLogRepo)
    const r = await uc.execute({ userId: 'u1', adminId: 'a1', reason: 'nope' })
